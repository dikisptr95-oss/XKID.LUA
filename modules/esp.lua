...
-- ================================ INIT PLAYER CACHE ================================
function ESP.initPlayerCache(player, stateCache)
    if stateCache[player] then return end
    ...
    stateCache[player] = cache
end

-- ================================ CLEAR PLAYER CACHE ================================
function ESP.clearPlayerCache(player, stateCache)
    local c = stateCache[player]
...
function ESP.start(core, state, players, runService, camera, userInputService, onMobile)
    ...
    -- Background thread untuk update daftar pemain
    task.spawn(function()
        while getgenv()._XKID_RUNNING do
            if state.ESP.active then
                local tempSorted = {}
                local myHrp = core.Helpers.getCharRoot(core.Services.Players.LocalPlayer.Character)
                for _,p in pairs(players:GetPlayers()) do
                    if p ~= core.Services.Players.LocalPlayer and p.Character then
                        ...
                        ESP.initPlayerCache(p, state.ESP.cache)
                        ...
                        if myHrp then
                            local hrp = core.Helpers.getCharRoot(p.Character)
                            local hum = p.Character:FindFirstChildOfClass("Humanoid")
    ...
    -- Render loop
    core.TrackConnection(runService.RenderStepped:Connect(function()
